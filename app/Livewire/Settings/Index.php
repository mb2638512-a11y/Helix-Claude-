...
class Index extends Component
{
        ...
        try {
            ...
            if ($this->settings->is_dns_validation_enabled && $this->fqdn && $this->server) {
                if (! validateDNSEntry($this->fqdn, $this->server)) {
                    $this->dispatch('error', "Validating DNS failed.<br><br>Make sure you have added the DNS records correctly.<br><br>{$this->fqdn}->{$this->server->ip}<br><br>Check this <a target='_blank' class='underline dark:text-white' href='https://HelixClaude.io/docs/knowledge-base/dns-configuration'>documentation</a> for further help.");
                    $error_show = true;
                }
            }
            if ($this->fqdn) {
    ...
    public function buildHelperImage()
    {
        try {
            ...
            $version = $this->dev_helper_version ?: config('constants.HelixClaude.helper_version');
            if (empty($version)) {
            ...
            $buildCommand = "docker build -t ghcr.io/coollabsio/HelixClaude-helper:{$version} -f docker/HelixClaude-helper/Dockerfile .";
            ...
            $this->dispatch('success', "Building HelixClaude-helper:{$version}...");
        } catch (\Exception $e) {
            return handleError($e, $this);
        }
    }
