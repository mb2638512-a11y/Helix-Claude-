...
class ByHetzner extends Component
{
    ...
    private function createHetznerServer(string $token): array
    {
        ...
        // Prepare SSH keys array: HelixClaude key + user-selected Hetzner keys
        $sshKeys = array_merge(
            [$sshKeyId], // HelixClaude key (always included)
            $this->selectedHetznerSshKeyIds // User-selected Hetzner keys
        );

        // Remove duplicates in case the HelixClaude key was also selected
    ...
    public function submit()
    {
        ...
        try {
            ...
            // Create server in HelixClaude database
