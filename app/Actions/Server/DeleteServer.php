...
class DeleteServer
{
    ...
    public function handle(int $serverId, bool $deleteFromHetzner = false, ?int $hetznerServerId = null, ?int $cloudProviderTokenId = null, ?int $teamId = null)
    {
        ...
        // Delete from Hetzner even if server is already gone from HelixClaude
        ...
        ray($server ? 'Deleting server from HelixClaude' : 'Server already deleted from HelixClaude, skipping HelixClaude deletion');

        // If server is already deleted from HelixClaude, skip this part
        if (! $server) {
            return; // Server already force deleted from HelixClaude
        }

        ray('force deleting server from HelixClaude', ['server_id' => $server->id]);

        try {
            $server->forceDelete();
        } catch (\Throwable $e) {
            ray('Failed to force delete server from HelixClaude', [
                'error' => $e->getMessage(),
                'server_id' => $server->id,
            ]);
            logger()->error('Failed to force delete server from HelixClaude', [
                'error' => $e->getMessage(),
                'server_id' => $server->id,
            ]);
        }
    ...
    private function deleteFromHetznerById(int $hetznerServerId, ?int $cloudProviderTokenId, int $teamId): void
    {
        try {
            ...
            // Log the error but don't prevent the server from being deleted from HelixClaude
