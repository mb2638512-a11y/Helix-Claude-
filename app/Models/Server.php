...
class Server extends BaseModel
{
    ...
    protected $appends = ['is_HelixClaude_host'];
    ...
    {
        $banner =
            "# This file is generated by HelixClaude, do not edit it manually.\n".
        ...
        if (isDev()) {
            if ($proxy_type === ProxyTypes::CADDY->value) {
                $dynamic_conf_path = '/data/HelixClaude/proxy/caddy/dynamic';
            }
        }
   ...
   redir $redirect_url
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/HelixClaude.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                ...
                $traefik_dynamic_conf = [
                    'http' => [
                        ...
                        'routers' => [
                            ...
                            'HelixClaude-realtime-ws' => [
                                'entryPoints' => [
                                    0 => 'http',
                                ],
                                'service' => 'HelixClaude-realtime',
                                'rule' => "Host(`{$host}`) && PathPrefix(`/app`)",
                            ],
                            'HelixClaude-terminal-ws' => [
                                'entryPoints' => [
                                    0 => 'http',
                                ],
                                'service' => 'HelixClaude-terminal',
                        ...
                        'services' => [
                            'HelixClaude' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://HelixClaude:8080',
                                        ],
                                    ],
                                ],
                            ],
                            'HelixClaude-realtime' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://HelixClaude-realtime:6001',
                                        ],
                                    ],
                                ],
                            ],
                            'HelixClaude-terminal' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://HelixClaude-realtime:6002',
                                        ],
                                    ],
                                ],
                            ],
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['HelixClaude-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['HelixClaude-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'HelixClaude',
                    ...
                    $traefik_dynamic_conf['http']['routers']['HelixClaude-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'HelixClaude-realtime',
                    ...
                    $traefik_dynamic_conf['http']['routers']['HelixClaude-terminal-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'HelixClaude-terminal',
                ...
                $yaml =
                    "# This file is automatically generated by HelixClaude.\n".
            ...
            $file = "$dynamic_config_path/HelixClaude.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy HelixClaude-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy HelixClaude-realtime:6002
    }
    reverse_proxy HelixClaude:8080
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec HelixClaude-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('HelixClaude.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        ...
        return $postgresqls
            ...
            ->concat($dragonflies)
            ->concat($clickhouses)
            ->filter(fn ($item) => data_get($item, 'name') !== 'HelixClaude-db');
    }
    ...
    public function validateHelixClaudeNetwork($isSwarm = false, $isBuildServer = false)
    {
        ...
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay HelixClaude-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create HelixClaude --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
