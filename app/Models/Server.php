...
class Server extends BaseModel
{
    ...
    protected $appends = ['is_Helix Claude_host'];
    ...
    {
        $banner =
            "# This file is generated by Helix Claude, do not edit it manually.\n".
        ...
        if (isDev()) {
            if ($proxy_type === ProxyTypes::CADDY->value) {
                $dynamic_conf_path = '/data/Helix Claude/proxy/caddy/dynamic';
            }
        }
   ...
   redir $redirect_url
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/Helix Claude.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                    ...
                    'http' => [
                        ...
                        'routers' => [
                            'Helix Claude-http' => [
                                ...
                                'service' => 'Helix Claude',
                                'rule' => "Host(`{$host}`)",
                            ],
                            'Helix Claude-realtime-ws' => [
                                'entryPoints' => [
                                    0 => 'http',
                                ],
                                'service' => 'Helix Claude-realtime',
                                'rule' => "Host(`{$host}`) && PathPrefix(`/app`)",
                            ],
                            'Helix Claude-terminal-ws' => [
                                'entryPoints' => [
                                    0 => 'http',
                                ],
                                'service' => 'Helix Claude-terminal',
                        ...
                        'services' => [
                            'Helix Claude' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://Helix Claude:8080',
                                        ],
                                    ],
                                ],
                            ],
                            'Helix Claude-realtime' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://Helix Claude-realtime:6001',
                                        ],
                                    ],
                                ],
                            ],
                            'Helix Claude-terminal' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://Helix Claude-realtime:6002',
                                        ],
                                    ],
                                ],
                            ],
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['Helix Claude-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['Helix Claude-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'Helix Claude',
                    ...
                    $traefik_dynamic_conf['http']['routers']['Helix Claude-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'Helix Claude-realtime',
                    ...
                    $traefik_dynamic_conf['http']['routers']['Helix Claude-terminal-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'Helix Claude-terminal',
                ...
                $yaml =
                    "# This file is automatically generated by Helix Claude.\n".
            ...
            $file = "$dynamic_config_path/Helix Claude.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy Helix Claude-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy Helix Claude-realtime:6002
    }
    reverse_proxy Helix Claude:8080
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec Helix Claude-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('Helix Claude.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        ...
        return $postgresqls
            ...
            ->concat($dragonflies)
            ->concat($clickhouses)
            ->filter(fn ($item) => data_get($item, 'name') !== 'Helix Claude-db');
    }
    ...
    public function validateHelix ClaudeNetwork($isSwarm = false, $isBuildServer = false)
    {
        ...
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay Helix Claude-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create Helix Claude --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
