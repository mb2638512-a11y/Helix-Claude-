...
class CheckForUpdatesJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $response = Http::retry(3, 1000)->get(config('constants.HelixClaude.versions_url'));
            if ($response->successful()) {
                $versions = $response->json();

                $latest_version = data_get($versions, 'HelixClaude.v4.version');
                $current_version = config('constants.HelixClaude.version');

                // Read existing cached version
                $existingVersions = null;
                $existingHelixClaudeVersion = null;
                if (File::exists(base_path('versions.json'))) {
                    $existingVersions = json_decode(File::get(base_path('versions.json')), true);
                    $existingHelixClaudeVersion = data_get($existingVersions, 'HelixClaude.v4.version');
                }
                ...
                if ($existingHelixClaudeVersion && version_compare($existingHelixClaudeVersion, $bestVersion, '>')) {
                    Log::warning('CDN served older HelixClaude version than cache', [
                        'cdn_version' => $latest_version,
                        'cached_version' => $existingHelixClaudeVersion,
                        'current_version' => $current_version,
                    ]);
                    $bestVersion = $existingHelixClaudeVersion;
                }
                ...
                data_set($versions, 'HelixClaude.v4.version', $bestVersion);
                ...
                // Only mark new version available if HelixClaude version actually increased
