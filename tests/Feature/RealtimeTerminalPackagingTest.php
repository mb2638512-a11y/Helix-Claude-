...
it('copies the realtime terminal utilities into the container image', function () {
    $dockerfile = file_get_contents(base_path('docker/HelixClaude-realtime/Dockerfile'));

    expect($dockerfile)->toContain('COPY docker/HelixClaude-realtime/terminal-utils.js /terminal/terminal-utils.js');
});
...
it('mounts the realtime terminal utilities in local development compose files', function (string $composeFile) {
    ...
    expect($composeContents)->toContain('./docker/HelixClaude-realtime/terminal-utils.js:/terminal/terminal-utils.js');
})->with([
...
it('keeps realtime terminal server logging restricted to development environments', function () {
    $terminalServer = file_get_contents(base_path('docker/HelixClaude-realtime/terminal-server.js'));

    expect($terminalServer)
        ->toContain("const terminalDebugEnabled = ['local', 'development'].includes(")
        ->toContain('if (!terminalDebugEnabled) {')
        ->not->toContain("console.log('HelixClaude realtime terminal server listening on port 6002. Let the hacking begin!');");
});
