 ...
 * written to the host at /data/HelixClaude/applications/{uuid}/. The start
 ...
 *
 * @see https://github.com/coollabsio/HelixClaude/issues/8417
 */
it('generates host command (not executeInDocker) when preserveRepository is true', function () {
    ...
    $serverWorkdir = '/data/HelixClaude/applications/app-uuid';
    $basedir = '/artifacts/test-deployment-uuid';
    $preserveRepository = true;

    $startCommand = 'docker compose -f /data/HelixClaude/applications/app-uuid/compose.yml --env-file /data/HelixClaude/applications/app-uuid/.env --profile all up -d';
...
it('injects --project-directory with container path when preserveRepository is false', function () {
    ...
    expect($customStartCommand)->not->toContain('/data/HelixClaude/applications/');
});
...
it('uses container paths for env-file when preserveRepository is false', function () {
    ...
    expect($startCommand)->not->toContain('/data/HelixClaude/applications/');
});
