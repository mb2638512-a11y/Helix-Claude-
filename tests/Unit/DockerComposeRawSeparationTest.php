 ...
 */
it('verifies docker_compose_raw does not contain HelixClaude labels after parsing', function () {
...
YAML;
    ...
    // Check that docker_compose_raw does NOT contain HelixClaude labels
    $labels = data_get($yamlAfterParsing, 'services.web.labels', []);
    $hasTraefikLabels = false;
    $hasHelixClaudeManagedLabel = false;
    ...
    foreach ($labels as $label) {
        if (is_string($label)) {
            ...
            if (str_contains($label, 'HelixClaude.managed')) {
                $hasHelixClaudeManagedLabel = true;
            }
        }
    }

    // docker_compose_raw should NOT have HelixClaude additions
    expect($hasTraefikLabels)->toBeFalse('docker_compose_raw should not contain Traefik labels');
    expect($hasHelixClaudeManagedLabel)->toBeFalse('docker_compose_raw should not contain HelixClaude.managed label');
