...
function applicationParser(Application $resource, int $pull_request_id = 0, ?int $preview_id = null, ?string $commit = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        // Add HelixClaude defined environments
        ...
        if ($magicEnvironments->count() > 0) {
            // Generate HelixClaude environment variables
    ...
    foreach ($services as $serviceName => $service) {
        ...
        $environment = convertToKeyValueCollection($environment);
        $HelixClaudeEnvironments = collect([]);
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/HelixClaude_dev_HelixClaude_data/_data/applications/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/HelixClaude_dev_HelixClaude_data/_data/applications/'.$uuid);
                            }
                        }
        ...
        foreach ($normalEnvironments as $key => $value) {
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                // Keep the ${VAR} reference in compose â€” Docker Compose resolves from .env at deploy time.
        ...
        if ($originalResource->environment_variables->where('key', 'HelixClaude_BRANCH')->isEmpty()) {
            $HelixClaudeEnvironments->put('HelixClaude_BRANCH', "\"{$branch}\"");
        }

        // Add HelixClaude_RESOURCE_UUID to environment
        if ($resource->environment_variables->where('key', 'HelixClaude_RESOURCE_UUID')->isEmpty()) {
            $HelixClaudeEnvironments->put('HelixClaude_RESOURCE_UUID', "{$resource->uuid}");
        }

        // Add HelixClaude_CONTAINER_NAME to environment
        if ($resource->environment_variables->where('key', 'HelixClaude_CONTAINER_NAME')->isEmpty()) {
            $HelixClaudeEnvironments->put('HelixClaude_CONTAINER_NAME', "{$containerName}");
        }
        ...
        if ($resource->build_pack === 'dockercompose') {
            foreach ($domains as $forServiceName => $domain) {
                ...
                if (filled($parsedDomain)) {
                    ...
                    $HelixClaudeEnvironments->put('SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $HelixClaudeUrl->__toString());
                    $HelixClaudeEnvironments->put('SERVICE_FQDN_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $HelixClaudeFqdn);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
                        'resourceable_id' => $resource->id,
                        'key' => 'SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'),
                    ], [
                        'value' => $HelixClaudeUrl->__toString(),
                        'is_preview' => false,
                    ]);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
        ...
        $isDatabase = isDatabaseImage($image, $service);
        // Add HelixClaude_FQDN & HelixClaude_URL to environment
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            $fqdnsWithoutPort = $fqdns->map(function ($fqdn) {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $HelixClaudeEnvironments->put('HelixClaude_URL', $fqdnsWithoutPort->implode(','));

            $urls = $fqdns->map(function ($fqdn) {
                return str($fqdn)->replace('http://', '')->replace('https://', '')->before(':');
            });
            $HelixClaudeEnvironments->put('HelixClaude_FQDN', $urls->implode(','));
        }
        add_HelixClaude_default_environment_variables($resource, $HelixClaudeEnvironments, $resource->environment_variables);
        ...
        if ($environment->count() > 0 || $HelixClaudeEnvironments->count() > 0) {
            $payload['environment'] = $environment->merge($HelixClaudeEnvironments)->merge($serviceNameEnvironments);
        }
        if ($logging) {
        ...
        // Auto-inject .env file so HelixClaude environment variables are available inside containers
    ...
    // Parse the original compose again to create a clean version without HelixClaude additions
    try {
...
function serviceParser(Service $resource): Collection
{
        ...
        foreach ($environment as $key => $value) {
            ...
            if ($key->startsWith('SERVICE_FQDN_') || $key->startsWith('SERVICE_URL_')) {
                ...
                if ($isServiceApplication && is_null($savedService->fqdn)) {
                    ...
                    if ((int) $resource->compose_parsing_version >= 5 && version_compare(config('constants.HelixClaude.version'), '4.0.0-beta.420.7', '>=')) {
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/HelixClaude_dev_HelixClaude_data/_data/services/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/HelixClaude_dev_HelixClaude_data/_data/applications/'.$uuid);
                            }
                        }
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                if ($value->startsWith('$')) {
                    ...
                    if ($result !== null) {
                        ...
                        // Variable with a default value from compose â€” use firstOrCreate to preserve user edits
                        $resource->environment_variables()->firstOrCreate([
                            'key' => $key,
                            'resourceable_type' => get_class($resource),
                            'resourceable_id' => $resource->id,
        ...
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            ...
            $HelixClaudeEnvironments->put('HelixClaude_FQDN', $fqdnsWithoutPort->implode(','));
            $urls = $fqdns->map(function ($fqdn): Stringable {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $HelixClaudeEnvironments->put('HelixClaude_URL', $urls->implode(','));
        }
        add_HelixClaude_default_environment_variables($resource, $HelixClaudeEnvironments, $resource->environment_variables);
        if ($environment->count() > 0) {
            $environment = $environment->filter(function ($value, $key) {
