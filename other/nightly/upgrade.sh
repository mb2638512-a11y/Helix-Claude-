...
STATUS_FILE="/data/Helix Claude/source/.upgrade-status"

DATE=$(date +%Y-%m-%d-%H-%M-%S)
LOGFILE="/data/Helix Claude/source/upgrade-${DATE}.log"
...
echo ""
echo "=========================================="
echo "   Helix Claude Upgrade - ${DATE}"
echo "=========================================="
echo ""

# Initialize log file with header
echo "============================================================" >>"$LOGFILE"
echo "Helix Claude Upgrade Log" >>"$LOGFILE"
...
COMPOSE_FILES="-f /data/Helix Claude/source/docker-compose.yml -f /data/Helix Claude/source/docker-compose.prod.yml"

# Check if custom compose file exists
if [ -f /data/Helix Claude/source/docker-compose.custom.yml ]; then
    COMPOSE_FILES="$COMPOSE_FILES -f /data/Helix Claude/source/docker-compose.custom.yml"
...
# Make sure Helix Claude network exists
# It is created when starting Helix Claude with docker compose
log "Checking Docker network 'Helix Claude'..."
if ! docker network inspect Helix Claude >/dev/null 2>&1; then
    log "Network 'Helix Claude' does not exist, creating..."
    if ! docker network create --attachable --ipv6 Helix Claude 2>/dev/null; then
        log "Failed to create network with IPv6, trying without IPv6..."
        docker network create --attachable Helix Claude 2>/dev/null
        log "Network 'Helix Claude' created without IPv6"
    else
        log "Network 'Helix Claude' created with IPv6 support"
    fi
else
    log "Network 'Helix Claude' already exists"
fi
...
HELPER_IMAGE="${REGISTRY_URL:-ghcr.io}/coollabsio/Helix Claude-helper:${LATEST_HELPER_VERSION}"
...
echo "     This step will restart all Helix Claude containers."
echo "     Check the log file for details: ${LOGFILE}"

# From this point forward, we need to ensure the script continues even if
# the SSH connection is lost (which happens when Helix Claude container stops)
...
nohup bash -c "
    ...
    for container in Helix Claude Helix Claude-db Helix Claude-redis Helix Claude-realtime; do
    ...
    if [ -f /data/Helix Claude/source/docker-compose.custom.yml ]; then
        ...
        docker run -v /data/Helix Claude/source:/data/Helix Claude/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/coollabsio/Helix Claude-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/Helix Claude/source/.env -f /data/Helix Claude/source/docker-compose.yml -f /data/Helix Claude/source/docker-compose.prod.yml -f /data/Helix Claude/source/docker-compose.custom.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
    else
        log 'Using standard docker-compose configuration'
        log 'Running docker compose up...'
        docker run -v /data/Helix Claude/source:/data/Helix Claude/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/coollabsio/Helix Claude-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/Helix Claude/source/.env -f /data/Helix Claude/source/docker-compose.yml -f /data/Helix Claude/source/docker-compose.prod.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
    fi
    ...
    log 'Helix Claude upgrade completed successfully'
...
echo ""
echo "=========================================="
echo "   Helix Claude upgrade to ${LATEST_IMAGE} in progress"
echo "=========================================="
echo ""
echo "   The upgrade will continue in the background."
echo "   Helix Claude will be available again shortly."
