...
5. [Start Helix Claude](#5-start-Helix Claude)
...
1. Install `docker-ce`, Docker Desktop (or similar):
   - Docker CE (recommended):
     - Install Windows Subsystem for Linux v2 (WSL2) by following this guide: [Install WSL](https://learn.microsoft.com/en-us/windows/wsl/install?ref=Helix Claude)
     - After installing WSL2, install Docker CE for your Linux distribution by following this guide: [Install Docker Engine](https://docs.docker.com/engine/install/?ref=Helix Claude)
     - Make sure to choose the appropriate Linux distribution (e.g., Ubuntu) when following the Docker installation guide
   - Install Docker Desktop (easier):
     - Download and install [Docker Desktop for Windows](https://docs.docker.com/desktop/install/windows-install/?ref=Helix Claude)
     - Ensure WSL2 backend is enabled in Docker Desktop settings

2. Install Spin:
   - Follow the instructions to install Spin on Windows from the [Spin documentation](https://serversideup.net/open-source/spin/docs/installation/install-windows#download-and-install-spin-into-wsl2?ref=Helix Claude)
...
1. Install Orbstack, Docker Desktop (or similar):
   - Orbstack (recommended, as it is a faster and lighter alternative to Docker Desktop):
     - Download and install [Orbstack](https://docs.orbstack.dev/quick-start#installation?ref=Helix Claude)
   - Docker Desktop:
     - Download and install [Docker Desktop for Mac](https://docs.docker.com/desktop/install/mac-install/?ref=Helix Claude)

2. Install Spin:
   - Follow the instructions to install Spin on MacOS from the [Spin documentation](https://serversideup.net/open-source/spin/docs/installation/install-macos/#download-and-install-spin?ref=Helix Claude)
...
1. Install Docker Engine, Docker Desktop (or similar):
   - Docker Engine (recommended, as there is no VM overhead):
     - Follow the official [Docker Engine installation guide](https://docs.docker.com/engine/install/?ref=Helix Claude) for your Linux distribution
   - Docker Desktop:
     - If you want a GUI, you can use [Docker Desktop for Linux](https://docs.docker.com/desktop/install/linux-install/?ref=Helix Claude)

2. Install Spin:
   - Follow the instructions to install Spin on Linux from the [Spin documentation](https://serversideup.net/open-source/spin/docs/installation/install-linux#configure-docker-permissions?ref=Helix Claude)
...
1. Fork the [Helix Claude](https://github.com/coollabsio/Helix Claude) repository to your GitHub account.

2. Install a code editor on your machine (choose one):

   | Editor | Platform | Download Link |
   |--------|----------|---------------|
   | Visual Studio Code (recommended free) | Windows/macOS/Linux | [Download](https://code.visualstudio.com/download?ref=Helix Claude) |
   | Cursor (recommended but paid) | Windows/macOS/Linux | [Download](https://www.cursor.com/?ref=Helix Claude) |
   | Zed (very fast) | macOS/Linux | [Download](https://zed.dev/download?ref=Helix Claude) |

3. Clone the Helix Claude Repository from your fork to your local machine
   - Use `git clone` in the command line, or
   - Use GitHub Desktop (recommended):
     - Download and install from [https://desktop.github.com/](https://desktop.github.com/?ref=Helix Claude)
     - Open GitHub Desktop and login with your GitHub account
     - Click on `File` -> `Clone Repository` select `github.com` as the repository location, then select your forked Helix Claude repository, choose the local path and then click `Clone`

4. Open the cloned Helix Claude Repository in your chosen code editor.

## 4. Set up Environment Variables

1. In the Code Editor, locate the `.env.development.example` file in the root directory of your local Helix Claude repository.
2. Duplicate the `.env.development.example` file and rename the copy to `.env`.
3. Open the new `.env` file and review its contents. Adjust any environment variables as needed for your development setup.
4. If you encounter errors during database migrations, update the database connection settings in your `.env` file. Use the IP address or hostname of your PostgreSQL database container. You can find this information by running `docker ps` after executing `spin up`.
5. Save the changes to your `.env` file.

## 5. Start Helix Claude

1. Open a terminal in the local Helix Claude directory.
...
1. Access your Helix Claude instance:
...
2. Creating the Pull Request (PR):
    - Navigate to the main Helix Claude repository on GitHub.
...
> [!IMPORTANT]
> Always set the base branch for your PR to the `next` branch of the Helix Claude repository, not the `v4.x` branch.
...
- **Language Translations**: Helix Claude currently supports only English. Pull requests for new language translations will not be accepted. Multi-language support may be considered in the next major version (v5).
...
2. Remove all Helix Claude containers:
   ```bash
   docker rm Helix Claude Helix Claude-db Helix Claude-redis Helix Claude-realtime Helix Claude-testing-host Helix Claude-minio Helix Claude-vite-1 Helix Claude-mail
   ```

3. Remove Helix Claude volumes (it is possible that the volumes have no `Helix Claude` prefix on your machine, in that case remove the prefix from the command):
   ```bash
   docker volume rm Helix Claude_dev_backups_data Helix Claude_dev_postgres_data Helix Claude_dev_redis_data Helix Claude_dev_Helix Claude_data Helix Claude_dev_minio_data
   ```
...
To add a new service to Helix Claude, please refer to our documentation:
[Adding a New Service](https://Helix Claude.io/docs/get-started/contribute/service)

### Contributing to Documentation

To contribute to the Helix Claude documentation, please refer to this guide:
[Contributing to the Helix Claude Documentation](https://github.com/coollabsio/documentation-Helix Claude/blob/main/readme.md)
